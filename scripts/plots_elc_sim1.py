...
directory = os.environ['LOCOSIM_DIR'] + '/landing_controller/simulations/ELCtests/20231203_135139/'
filename = "DATA.mat"
...
mpl.rcParams['figure.figsize'] = 9.2, 6.8
...
graph = {}
graph['x'] = time_log[start:end]-time_log[start]
graph['y'] = centroid[0,0]*np.ones_like(graph['x']) + comPoseW_log[0, touch_down_sample0]
graph['y'][:touch_down_sample0] = np.nan
graph['label'] = r'$\gamma^{x}$'
graph['color'] = 'plum'
graph['linestyle'] = '-'
graph['linewidth'] = 2
graph['legend_linewidth'] = 2
...
graph = {}
graph['x'] = time_log[start:end]-time_log[start]
graph['y'] = zmp[0,0]*np.ones_like(graph['x']) + comPoseW_log[0, touch_down_sample0]
graph['y'][:touch_down_sample0] = np.nan
graph['label'] = r'$u_{o}^{x}$'
graph['color'] = 'darkviolet'
graph['linestyle'] = '-'
graph['linewidth'] = 2
graph['legend_linewidth'] = 2
plot1.append(graph)
...
ax1.legend(handles=legend_elements, loc='center right', handlelength=1.6, handleheight=1.5, labelspacing=0.05, ncol=2,
          columnspacing=0.8)
...
for k in range(touch_down_sample0, time_log.shape[0]-start-1):
    s = (k - touch_down_sample0)/(time_log.shape[0]-touch_down_sample0)
    graph['y'][k] =  graph['y'][k] + 40*np.exp(-.4*s)
####  lf grf
# x
graph = {}
graph['x'] = time_log[start:end]-time_log[start]
