from .ext_landingController2 import *
from landing_controller.controller.utility import makePlots, saveInitConds
import copy
import rospy as ros
import sys

class ExtendedLandingManager:
    ...
    def run(self, simulation_counter, basePose_init=None, baseTwist_init=None, useIK=False, useWBC=True, typeWBC='projection', naive=False):
        ...
        if self.p.real_robot:
            sys.exit("Do not use real_robot = True!")    
            # q_des = self.p.q.copy()
            ...
            self.elc = ExtendedLandingController(robot=self.p.robot,
                                                 dt=2 * self.p.dt,
                                                 q0=np.hstack([self.p.u.linPart(self.p.basePoseW),
                                                                self.p.quaternion,
                                                                q_des]),
            ...
            delta_expected_td = np.sqrt(2*(self.p.basePoseW[2]-0.25)/self.elc.g_mag)
            self.elc.setCheckTimings(expected_lift_off_time=None,
                                    expected_apex_time=None,
                                    expected_touch_down_time=self.p.time + delta_expected_td,
                                    clearance=0.05) # about 12 mm of error in touch down

            expected_velocity_td = self.p.u.linPart(self.p.TwistPoseW).copy()
            expected_velocity_td[2] -= self.elc.g_mag * delta_expected_td
            self.elc.elc_solve(expected_velocity_td)
            ...
            self.p.unpause_physics_client()
            self.p.setGravity(-self.elc.g_mag)
        ...
        while not ros.is_shutdown():
            ...
            self.p.updateKinematics(update_legOdom=self.elc.lc_events.touch_down.detected, noise=self.noise)
            ...
            if fsm_state == 1:
                ...
                if self.p.real_robot:
                    self.elc.touchDownReal(t=self.p.time,
                                          sample=self.p.log_counter,
                                          contacts_state=self.p.contact_state)
                else:
                    self.elc.touchDown(t=self.p.time,
                                      sample=self.p.log_counter,
                                      contacts_state=self.p.contact_state)

                if self.elc.lc_events.touch_down.detected:
                    fsm_state += 1
                    height = 0.
                    for leg in range(4):
                    ...
                    self.elc.landed(comPoseH, comRateH)
                    ...
                    if self.p.real_robot:
                        self.elc.flyingDown_phase(self.p.b_R_w @ w_R_hf, w_R_hf.T @ self.p.u.linPart(self.p.baseTwistW))
                    else:
                        baseLinVelW_init[2] = self.p.baseTwistW[2]
                        self.elc.flyingDown_phase(self.p.b_R_w @ w_R_hf, w_R_hf.T @ baseLinVelW_init)
                    ...
                    for i, leg in enumerate(self.elc.legs):  # ['lf', 'rf', 'lh', 'lh']
                        ...
                        # joints velocity
                        B_contact_err = self.elc.B_feet_task[i] - self.p.B_contacts[i]
                        kv = .01 / self.p.dt
            ...
            #################################################################
            if fsm_state == 2:
                if self.elc.lp_counter > 2 * self.elc.ref_k + 100:  # take a bit before quitting
                    fsm_state = 3
                    break
                else:
                    # use last computed trajectories
                    self.elc.landed_phase(self.p.time)

                    # set references
                    b_R_w_des = pin.rpy.rpyToMatrix(self.p.u.angPart(self.elc.pose_des)).T
                    omega_des_skew = pin.skew(b_R_w_des @ self.p.u.angPart(self.elc.pose_des))
                    # joints position
                    for i, leg in enumerate(self.elc.legs):  # ['lf', 'lh', 'rf','rh']
                        q_des_leg, isFeasible = self.p.IK.ik_leg(self.elc.B_feet_task[i],
                                                                 leg,
                                                                 self.p.legConfig[leg][0],
                                                                 self.p.legConfig[leg][1],
                        ...
                        # joints velocity
                        self.p.B_vel_contacts_des[i] = omega_des_skew.T @ self.elc.B_feet_task[
                            i] - b_R_w_des @ self.p.u.linPart(self.elc.twist_des)
                    ...
                    self.p.comPoseW_des, self.p.comTwistW_des, comAccW_des = self.p.Hframe2World(self.elc.pose_des,
                                                                                                 self.elc.twist_des,
                                                                                                 self.elc.acc_des)
            ...
            for leg in range(4):
                self.p.W_contacts_des[leg] = self.p.u.linPart(self.p.basePoseW) + self.p.b_R_w.T @ self.elc.B_feet_task[leg]
                self.p.B_contacts_des[leg] = self.elc.B_feet_task[leg].copy()
        ...
        if not self.p.real_robot:
            ...
            if self.settings['WORKSPACE']['save']:
                EXTRADATA = {}
                EXTRADATA["lift_off_sample"] = self.elc.lc_events.lift_off.sample
                EXTRADATA["lift_off_t"] = self.elc.lc_events.lift_off.t
                EXTRADATA["apex_sample"] = self.elc.lc_events.apex.sample
                EXTRADATA["apex_t"] = self.elc.lc_events.apex.t
                EXTRADATA["touch_down_sample"] = self.elc.lc_events.touch_down.sample
                EXTRADATA["touch_down_t"] = self.elc.lc_events.touch_down.t
                self.p.saveData(self.settings['SIMS'][simulation_counter]['directory'], EXTRADATA=EXTRADATA,
                                start=self.elc.lc_events.apex.sample, stop=self.p.log_counter,
                                verbose = self.settings['verbose'])
